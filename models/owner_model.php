...
class owner_model extends Model{
    ...
    public function FindSumIncomeTransaction($Fdate,$Ldate,$t1,$t3){
        $sql ="SELECT sum(amount) as income FROM transaction_detail where (Transaction_type = :t1 || Transaction_type = :t3) and (Time BETWEEN  :Fdate and :Ldate)";
        $pstm = $this->connect->prepare($sql);
        $pstm->bindParam(':Fdate',$Fdate);
        $pstm->bindParam(':Ldate',$Ldate);
        $pstm->bindParam(':t1',$t1);
    ...
    public function FindSumExpensesTransaction($Fdate,$Ldate,$t2,$t4){
        $sql ="SELECT sum(amount) as expenses FROM transaction_detail where (Transaction_type = :t2 || Transaction_type = :t4) and (Time BETWEEN  :Fdate and :Ldate)";
        $pstm = $this->connect->prepare($sql);
        $pstm->bindParam(':Fdate',$Fdate);
        $pstm->bindParam(':Ldate',$Ldate);
        $pstm->bindParam(':t2',$t2);
